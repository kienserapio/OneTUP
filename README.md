...
The palette is Apple's system colour set with one saturated brand colour, TUP
crimson, used sparingly enough that it always means something. SF Pro is
referenced through `-apple-system` and `local()` rather than shipped — Apple
licenses it for interfaces on Apple platforms, not as a self-hosted webfont — so
Apple devices get the real thing and everything else falls back to Inter.
