...
import { spring, transition, prefersReducedMotion } from '@/design/motion'
 ...
 * at a time on the right. Two thresholds do two different jobs: 0.6 means a card
 * is the one being read, so the index follows it, and 0.15 means a card has
 * entered, so it reveals. Reveals fire once — content that re-animates every
 * time it passes the fold is decoration.
 */
...
export function FeaturesSection() {
  ...
  // Read after mount: matchMedia is not available while rendering on the server,
  // and reading it during render would desync the first client paint.
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        for (const entry of entries) {
          ...
          if (entry.intersectionRatio >= 0.6) setActive(index)
        }
      },
      { threshold: [0.15, 0.6] },
    )
        ...
        <div className="mt-16 flex flex-col gap-12 md:gap-20 lg:mt-0 lg:py-32">
          {FEATURES.map((feature, index) => (
            <article
              ...
              className={cx(
                'card squircle scroll-mt-24 p-5 duration-700 ease-out md:p-8',
                // Reduced motion keeps the fade and drops the travel — the
                // reveal still says "this is new", it just does not move.
                reduced ? 'transition-opacity' : 'transition',
                revealed
